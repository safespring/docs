...
!!! note "Safespring Storage"
    Safespring Storage is a S3 compatible object storage api. A standard tier is offered with low latency and high throughput. There is also an archive tier for long-term archival storage with lower costs. See [Quotas](../quota.md) for details on the differences between the tiers.
...
!!! note "Control numbering"
    ...
    number means that this site does not publish a Storage-specific statement
    for that control.
