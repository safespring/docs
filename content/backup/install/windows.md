...
To see options and rules to run simply run `"safespring-baas.2.4.3.exe -help"`. All installer parameters must be in CAPITAL letters. Flags can be in lower case.
...
[https://api.cloud.ipnett.se/dist/installer/safespring-baas.2.4.3.exe](https://api.cloud.ipnett.se/dist/installer/safespring-baas.2.4.3.exe).

For older Windows versions please use this installer,
[https://api.cloud.ipnett.se/dist/installer/safespring-baas.2.2.3.exe](https://api.cloud.ipnett.se/dist/installer/safespring-baas.2.2.3.exe).

md5: b619287659f0d84caa999002fc3c16a7  safespring-baas.2.4.3.exe   
md5: e214804ae968770ed8ffe7a60f0d9097  safespring-baas.2.2.3.exe  
...
!!! note ""
    `safespring-baas.2.4.3.exe PASS=<password> (-passive | -silent)`
...
Put the safespring-baas.2.4.3.exe in a directory where you have write permissions since it will write the `dsm-<nodename>.zip` file in the current directory.

**Example:**

!!! note ""
    `safespring-baas.2.4.3.exe TOKEN=<secrettoken> FQDN=<uniquename> MAIL=<mailaddress> COST=<costcenter> (-silent | -passive)`
...
`safespring-baas.2.4.3.exe UPDATE=1`.
...
TBMRONLY argument. `safespring-baas.2.4.3.exe TBMRONLY=1`. This method can also be used to license TBMR if it is trial or unlicensed for some reason.
