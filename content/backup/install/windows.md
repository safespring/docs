...
To see options and rules to run simply run `"safespring-baas.2.4.4.exe -help"`. All installer parameters must be in CAPITAL letters. Flags can be in lower case.
...
[https://api.cloud.ipnett.se/dist/installer/safespring-baas.2.4.4.exe](https://api.cloud.ipnett.se/dist/installer/safespring-baas.2.4.4.exe).
...
md5: 88caa2b579329629f7498a7964cfd0a3  safespring-baas.2.4.4.exe   
...
!!! note ""
    `safespring-baas.2.4.4.exe PASS=<password> (-passive | -silent)`
...
Put the safespring-baas.2.4.4.exe in a directory where you have write permissions since it will write the `dsm-<nodename>.zip` file in the current directory.

**Example:**

!!! note ""
    `safespring-baas.2.4.4.exe TOKEN=<secrettoken> FQDN=<uniquename> MAIL=<mailaddress> COST=<costcenter> (-silent | -passive)`
...
`safespring-baas.2.4.4.exe UPDATE=1`.
...
TBMRONLY argument. `safespring-baas.2.4.4.exe TBMRONLY=1`. This method can also be used to license TBMR if it is trial or unlicensed for some reason.
