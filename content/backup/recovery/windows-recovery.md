...
* The client machine being protected must be running Windows.
* The machine where the ISO file will be created must be running 
  Windows Server 2022 or later **(can be the same as the client machine being 
  protected, but doesn't have to be)**. Windows 11 is also supported.
* The TSM Backup-Archive client (dsmc) must be installed and configured. See the 
  [Quickstart Guide](../quickstart-guide.md) for information about how to do this.
* A license for TBMR. 
  As a customer, you have access to our site license. Please contact 
  [support](../../service/support.md) to receive a copy of the TBMR software and license if you 
...
1. Run SetupTBMRSuite.exe and select which components to install.
    1. If you have not generated a CRISP Recovery ISO before, and wish to do so on
       this machine, make sure that 
       "CRISP WinPE11 Recovery Environment Fileset" is selected. 
       _This is not required
       for protecting the current machine, only for creating the ISO. The ISO
       can be created on a separate machine if you want, 
       in which case you will have to install CRISP there instead. Either way, you will need an ISO to recover a system using TBMR in case of a disaster._
    2. If you have generated a CRISP Recovery ISO before, and only intend to protect   this machine, select only "Cristie TBMR" (the first option).
...
2. Make sure to configure the license key using the Command Prompt _with 
   ...
   given to you by Safespring Support.
...
4. The special folder created by TBMR called `C:\TBMRCFG` contains 
   ...
   running on. The Recovery ISO will restore and use this information to make 
   sure that the system is recovered completely with all its
   partitions, and is made bootable. It is therefore important that the folder
   is generated and backed up before a disaster recovery. 
   You can make sure that this folder is generated and backed up immediately
   without having to wait for the next scheduled backup by running 
   (as an Administrator):
   ```
...
1. Install Microsoft Windows Assessment and Deployment Kit (WADK). The correct version (10.1.26100.2454) can be 
   downloaded from [Microsoft Download Center](https://go.microsoft.com/fwlink/?linkid=2289980).
   Make sure to install the following feature:
    - Deployment Tools
2. Install the [Microsoft Windows PE add-on](https://go.microsoft.com/fwlink/?linkid=2289981) as well.
   Make sure to install the following feature:
    - Windows Preinstallation Environment (Windows PE)
3. Open the "Start CRISP" application.
4. Select "Build WinPE11 recovery ISO/USB media for TBMR".
